...
template<typename T> double calculate_means(vector<int>& belongs_to, vmean& means, const vector<vector<T>>& points, vector<bool>& disappeared) {
    ...
    forn(i, k) {
        if(disappeared[i]) continue;
        if(point_count[i] == 0) {
            // TODO: Split clusters, k-means has gone wrong.
            disappeared[i] = true;
...
template<typename T> void assign_means(vector<int>& belongs_to, const vmean& means, const vector<vector<T>>& points, vector<bool>& disappeared) {
...
template<typename T> void initialize(vector<int>& belongs_to, vmean& means, const vector<vector<T>>& points, int k, vector<bool>& disappeared) {
    int n = points.size(), dim = points[0].size();
    belongs_to.resize(n, 0);
    means.resize(k, vector<double>(dim, 0));

    mt19937 gen(static_cast<uint>(time(0)));
    uniform_int_distribution<uint> partitions(0, k-1);
    for(int& x : belongs_to) x = partitions(gen);
    calculate_means(belongs_to, means, points, disappeared);
}
...
template<typename T> vector<int> k_means(const vector<vector<T>>& points, int k, double threshold) {
    ...
    vector<bool> disappeared(k);
    double dist;
    initialize(belongs_to, means, points, k, disappeared);
    do {
        assign_means(belongs_to, means, points, disappeared);
        dist = calculate_means(belongs_to, means, points, disappeared);
